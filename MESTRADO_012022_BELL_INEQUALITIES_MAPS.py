    ...
    for N in N_list:                                
        ...
        if N == 1:            
            ...
            for Omega in Omega_list:
                ...
                path_data_1 = "../results/data/QT/Matrix_hdf12_Bell_Inequalities_2_N=%d_Omega=%.2f_Delta=%.2f/"%(N,Omega,Delta)
                
                filename_1 = path_data_1+filename_generator.filename_gen_sensor("I",N,S,k,d,Omega,Delta)
                ...
                filename_2 =  path_data_2 + filename_generator.filename_gen_sensor("I",N,S,k,d,Omega,Delta)
                ...
                for i in range(0,omega_points):
                    for j in range(0,omega_points):                        
                        if B_s_Matrix[i,j] == True and g2_Matrix[i,j] > 1:
                            M[i,j] = g2_Matrix[i,j]
                        else: 
                            M[i,j] = float('nan')
        ...
        if N == 2:
            ...
            for distance in distance_list:                    
                ...
                for Omega in Omega_list:
                    ...
                    path_data_1 = "../results/data/QT/Matrix_hdf12_Bell_Inequalities__2_N=%d_kd=%.2f_Omega=%.2f_Delta=%.2f/"%(N,k0*d,Omega,Delta)
                        
                    filename_1 = path_data_1+filename_generator.filename_gen_sensor("BI",N,S,k,d,Omega,Delta)
                    f = h5py.File(filename_1,'r')
                    B_s_Matrix = f['/1/Bs']
                    ...
                    for i in range(0,omega_points):
                        for j in range(0,omega_points):                            
                            if B_s_Matrix == True and g2_Matrix[i,j] > 2:
                                M[i,j] = g2_Matrix[i,j]
                            else: 
                                M[i,j] = float('nan')
