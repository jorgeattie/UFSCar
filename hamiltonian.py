...
def Hamiltonian_gen(N,R,g,Omega,Delta,k_vec,level,F,S):
    H1,H2,H3 = 0,0,0
    for i in range(0,N):
...
def Hamiltonian_Sensors(N,k,n_hat,R,level,F,omega_sensor_list,epsilon):
    ...
    for i in range(N,F):
        ...
        H_s_2 += ksiM*E_m + ksiP*E_p        
    H_s = H_s_1 + epsilon*H_s_2        
    return H_s

def Hamiltonian_Sensors_bosonic(N,k,n_hat,R,level,F,omega_sensor_list,epsilon):
    E_p = electric_field.E_gen(N,level,F,'plus',n_hat,R,k)
    E_m = electric_field.E_gen(N,level,F,'minus',n_hat,R,k)
    H_s_1 = 0
    H_s_2 = 0
    ...
    for i in range(N,F):
        ...
        ksiP = sigma_plus.Sigmap_gen(level,F,i)
        ksiM = sigma_minus.Sigmam_gen(level,F,i)
        H_s_1 += omega_sensor*ksiM*ksiP        
        H_s_2 += ksiP*E_m + ksiM*E_p
    H_s = H_s_1 + epsilon*H_s_2
    return H_s
